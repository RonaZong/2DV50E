...
def make_prediction(model, loader, output_csv="sample.csv"):
    preds = []
    file_names = []
    model.eval()
    ...
    df = pd.DataFrame({"image": file_names, "level": np.concatenate(preds, axis=0)})
...
def get_csv_for_blend(loader, model, output_csv_file):
    warnings.warn("Important to hve shuffle=False (and to ensure batch size is even size) when running get_csv_for_blend also set val_transforms to train_loader!")
    model.eval()
    filename_first = []
    filename_second = []
    labels_first = []
    ...
    for idx, (images, y, image_files) in enumerate(tqdm(loader)):
        ...
        with torch.no_grad():
            ...
            preds = model(images).reshape(images.shhape[0] // 2, 2, 1)
            new_features = (
    ...
    df = pd.DataFrame(data=all_features, columns=[f"f_{idx}" for idx in range(all_features.shape[1])])
